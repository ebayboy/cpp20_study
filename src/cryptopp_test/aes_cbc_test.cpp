...
std::string CBC_AESDecryptStr(std::string sKey, std::string sIV,
    ...
    CBC_Mode<AES>::Decryption cbcDecryption((CryptoPP::byte *)key, AES::MAX_KEYLENGTH, iv);
...
int main(int argc, char **argv) {
    ...
    std::string encoded = CBC_AESEncryptStr(key, iv, data.data());
    fmt::println("encoded:{}", encoded);

    std::string decoded = CBC_AESDecryptStr(key, iv, encoded.data());
